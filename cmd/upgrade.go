...
import (
	...
	"explorer451/internal/migrations"
...
func upgrade(db *sqlx.DB, config *config.Config, prompt bool) {
	...
	if len(toRun) == 0 {
		...
		log.Printf("Found %d pending upgrade(s) to run.", len(toRun))
		for _, m := range toRun {
	...
	if err := createDefaultAdminUserIfNeeded(db, log.Default()); err != nil {
		log.Fatalf("Error creating default admin user during upgrade: %v", err)
	}
...
func checkUpgrade(db *sqlx.DB) {
	...
	log.Fatalf("there are %d pending database upgrade(s): %v. The last upgrade was %s. Backup the database and run explorer451 --upgrade",
