...
func initOIDC(a *Auth) {
	...
	a.log.Info().Msgf("OIDC Authentication enabled with provider: %s", a.oidcCfg.ProviderURL)
}
...
func startSessionPruner(ctx context.Context, a *Auth) {
	go func() {
		...
		for {
			select {
			case <-ticker.C:
				a.log.Debug().Msg("Pruning expired sessions")
				err := a.sessStore.Prune()
				if err != nil {
					a.log.Error().Err(err).Msg("Error pruning login sessions")
				} else {
					a.log.Debug().Msg("Expired session pruning completed")
				}
			case <-ctx.Done():
				a.log.Info().Msg("Session pruning goroutine shutting down")
				return
			}
		}
	}()
...
func startTokenPruner(ctx context.Context, a *Auth) {
	go func() {
		...
		for {
			select {
			case <-ticker.C:
				a.log.Debug().Msg("Pruning expired API tokens")
				count, err := a.repository.PruneExpiredTokens(context.Background())
				if err != nil {
					a.log.Error().Err(err).Msg("Error pruning expired tokens")
				} else if count > 0 {
					a.log.Info().Msgf("Pruned %d expired API tokens", count)
				}
			case <-ctx.Done():
				a.log.Info().Msg("Token pruning goroutine shutting down")
				return
			}
		}
	}()
...
func New(ctx context.Context, repository storage.Repository, cfg config.Config, db *sql.DB, cb *Callbacks, log *logger.Logger) (*Auth, error) {
	a := &Auth{
...
func (a *Auth) GetAPIToken(tokenValue string) (*models.Token, bool) {
	...
	if ok {
		...
		if token.ExpiresAt.Valid && token.ExpiresAt.Time.Before(time.Now()) {
			a.log.Info().Msgf("API token from cache expired: %s (ID: %s, expired_at: %s)", token.Name, token.ID, token.ExpiresAt.Time)
		...
		a.log.Debug().Msgf("API token cache hit: %s (ID: %s)", token.Name, token.ID)
		return &token, true
	}

	// 2. Cache miss: Fetch from database
	a.log.Debug().Msgf("API token cache miss, fetching from DB: %s", tokenValue)
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second) // Add timeout for DB query
	defer cancel()

	dbToken, err := a.repository.GetTokenByValue(ctx, tokenValue)
	if err != nil {
		// If token is not found in DB, it's invalid
		if errors.Is(err, sql.ErrNoRows) {
			a.log.Warn().Msgf("API token not found in DB: %s", tokenValue)
			return nil, false // Token not found
		}
		// Handle other potential database errors
		a.log.Error().Err(err).Msg("Error fetching token from DB")
	...
	if dbToken.ExpiresAt.Valid && dbToken.ExpiresAt.Time.Before(time.Now()) {
		a.log.Info().Msgf("API token fetched from DB is expired: %s (ID: %s, expired_at: %s)", dbToken.Name, dbToken.ID, dbToken.ExpiresAt.Time)
	...
	a.Unlock()
	a.log.Info().Msgf("API token fetched from DB and cached: %s (ID: %s)", dbToken.Name, dbToken.ID)
...
func (a *Auth) updateTokenLastUsedAsync(tokenID string) {
	...
	defer cancel()
	if err := a.repository.UpdateTokenLastUsed(ctx, tokenID); err != nil {
		a.log.Error().Err(err).Msgf("Failed to update last used time for token ID %s", tokenID)
	}
}
...
func (a *Auth) authenticateAPIToken(c echo.Context) (*models.User, error) {
	...
	if err != nil {
		...
		a.log.Error().Err(err).Msgf("Error fetching user for API token %s", token.ID)
...
func (a *Auth) Middleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err != nil && !errors.Is(err, simplesessions.ErrInvalidSession) && !errors.Is(err, http.ErrNoCookie) {
			a.log.Error().Err(err).Msg("Session validation error")
...
func (a *Auth) validateSession(c echo.Context) (*simplesessions.Session, *models.User, error) {
	...
	if err != nil {
		...
		a.log.Error().Err(err).Msg("Error acquiring session")
	...
	if err != nil {
		...
		a.log.Warn().Msgf("Error getting user_id from session %s: %v", sess.ID(), err)
	...
	if err != nil {
		...
		if errors.Is(err, sql.ErrNoRows) {
			a.log.Warn().Msgf("Invalid user_id '%v' in session %s: %v", userIDVal, sess.ID(), err)
			_ = sess.Destroy()
			return nil, nil, simplesessions.ErrInvalidSession
		}
		// Other DB error fetching user
		a.log.Error().Err(err).Msgf("Error fetching user %s for session %s", userID, sess.ID())
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			a.log.Warn().Msgf("User %s not found for session %s: %v", userID, sess.ID(), err)
			_ = sess.Destroy()
			return nil, nil, simplesessions.ErrInvalidSession
		}
		a.log.Error().Err(err).Msgf("Error fetching user %s for session %s", userID, sess.ID())
...
func (a *Auth) Logout(c echo.Context) error {
	...
	err := sess.Destroy()
	if err != nil {
		a.log.Error().Err(err).Msg("Error destroying session")
		return echo.NewHTTPError(http.StatusInternalServerError, "Failed to logout")
	}
	a.log.Info().Msgf("User logged out, session destroyed: %s", sess.ID())
	return nil
}
